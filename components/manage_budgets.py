...
def render_budget_overview():
    ...
    for budget in budgets:
        ...
        with st.expander(f"🎯 {budget['category']} - {format_currency(budget['amount'])} ({budget['period']})"):
            ...
            with col3:
                if progress['spent'] > budget['amount']:
                    st.error(f"{get_text('budget.over_budget')} {format_currency(progress['spent'] - budget['amount'])}")
                elif progress['spent'] >= budget['amount'] * 0.8:
                    st.warning(get_text('budget.warning_threshold'))
            
            # Progress bar
            progress_percentage = min((progress['spent'] / budget['amount']) * 100, 100)
...
def render_manage_budgets():
    ...
    for budget in budgets:
        with st.expander(f"🎯 {budget['category']} - {format_currency(budget['amount'])} ({budget['period']})"):
            ...
            st.write(f"- {get_text('budget.notification')}: {budget['notification_threshold']*100}%")
